module Scraping.GovData.AberdeenSokKwuWan.Metadata () where
...
csv = withCache "GovData-AberdeenSokKwuWan-Metadata-CSV" $
    simpleHttp "https://www.td.gov.hk/filemanager/en/content_1408/opendata/ferry_abd_skw_faretable_eng.csv"

instance (MonadIO m, MonadCache m ByteString, MonadCache m Metadata) => HasMetadata m AberdeenSokKwuWan where
    fetchMetadata _ = withCache "GovData-AberdeenSokKwuWan-Metadata" $ do
        res <- csv
...
data Entry = Entry Passenger' Route' Days' Fare' Modifier'
...
toMetadata entries = Metadata { fares = fares, durations = durations }
    where
        ...
        entryToFare (Entry (Passenger' passenger') (Route' route) (Days' daySet) (Fare' fare) (Modifier' modifiers))
            | route == "Aberdeen - Sok Kwu Wan"
                = Just $ Fare
                    { passenger = passenger'
                    , days = daySet
                    , fare = fare
