module Scraping.GovData.CentralSokKwuWan.Metadata () where
...
csv = withCache "GovData-CentralSokKwuWan-Metadata-CSV" $
    simpleHttp "https://www.td.gov.hk/filemanager/en/content_1408/opendata/ferry_central_skw_faretable_eng.csv"

instance (MonadIO m, MonadCache m ByteString, MonadCache m Metadata) => HasMetadata m CentralSokKwuWan where
    fetchMetadata _ = withCache "GovData-CentralSokKwuWan-Metadata" $ do
        res <- csv
