module Scraping.GovData.PengChauHeiLingChau.Metadata () where
...
csv = withCache "GovData-PengChauHeiLingChau-Metadata-CSV" $
    simpleHttp "https://www.td.gov.hk/filemanager/en/content_1408/opendata/ferry_central_pc_faretable_eng.csv"

instance (MonadIO m, MonadCache m ByteString, MonadCache m Metadata) => HasMetadata m PengChauHeiLingChau where
    fetchMetadata _ = withCache "GovData-PengChauHeiLingChau-Metadata" $ do
        res <- csv
...
toMetadata entries = Metadata { fares = fares, durations = durations }
    where
        ...
        entryToFare (Entry (Route' "Peng Chau - Hei Ling Chau") (Passenger' passenger') (Days' daySet) (FareType' fareType) (Fare' fare) (Modifier' modifiers)) =
            Just $ Fare
                { passenger = passenger'
                , days = daySet
                , fare = fare
