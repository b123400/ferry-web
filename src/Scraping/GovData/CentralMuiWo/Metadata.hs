module Scraping.GovData.CentralMuiWo.Metadata () where
...
csv = withCache "GovData-CentralMuiWo-Metadata-CSV" $
    simpleHttp "https://www.td.gov.hk/filemanager/en/content_1408/opendata/ferry_central_mw_faretable_eng.csv"

instance (MonadIO m, MonadCache m ByteString, MonadCache m Metadata) => HasMetadata m CentralMuiWo where
    fetchMetadata _ = withCache "GovData-CentralMuiWo-Metadata" $ do
        res <- csv
...
toMetadata entries = Metadata { fares = fares, durations = durations }
    where
        ...
        durations =
            [ Duration (Just T.FastFerry) (40 * 60)
            , Duration Nothing (55 * 60)
            ]
